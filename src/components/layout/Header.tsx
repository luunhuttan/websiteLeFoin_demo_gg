...
export function Header() {
  ...
  return (
    <header className="fixed top-0 left-0 right-0 z-50 border-b border-water shadow-md bg-white/90 dark:bg-gray-900/90 backdrop-blur">
      ...
      <div className="container container-header flex h-16 items-center justify-between relative px-2 md:px-0">
        {/* Logo sát lề trái */}
        <div className="flex-none flex items-center min-w-[120px]">
          <Link href="/" className="flex items-center pl-0 ml-0">
            <Image 
              ...
              width={120} 
              height={40} 
              className="logo-lefoin h-10 w-auto"
              priority
        ...
        {/* Nút bên phải */}
        <div className="flex-none flex items-center gap-2 md:gap-4 min-w-[120px] justify-end">
          <LanguageSwitcher />
          <DarkModeButton />
          {user && user.email ? (
            <div className="relative" ref={userMenuRef}>
              ...
              {isUserMenuOpen && (
                <div className="absolute right-0 mt-2 w-52 bg-white dark:bg-gray-800 rounded-lg shadow-lg py-1 z-50 border border-gray-200 dark:border-gray-700">
      ...
      {/* Mobile Menu */}
      {isMobileMenuOpen && (
        <div className="md:hidden border-t border-water bg-water/95 dark:bg-gray-900/95 relative">
          <div className="header-gradient w-full h-full absolute inset-0 pointer-events-none"></div>
